...
const MyOrder = () => {
    ...
    useState(() => {
        fetch(`https://damp-mesa-00086.herokuapp.com/myOrder/${user?.email}`)
            .then(res => res.json())
            .then(data => {
                setMyOrder(data)
                setLoading(false)
    ...
    const hanldeOrderDelete = id => {
        ...
        if (process) {
            const url = `https://damp-mesa-00086.herokuapp.com/myOrder/${id}`
            fetch(url, {
                method: 'DELETE',
            })
                .then(res => res.json())
    ...
    return (
        <div className="container-fluid">
            <h2 className="text-center py-3 mb-4 fw-bold"> My Order </h2>
                    ...
                    myOder.map(order => <div
                        ...
                        <div className="card h-100">
                            ...
                            <div className="card-body text-center">
                                ...
                                <button onClick={() => hanldeOrderDelete(order._id)} className="btn-cancel my-3">Cancel Order</button>
                            </div>
                        </div>
                    </div>)
                }
