...
const Purchase = () => {
    ...
    const handleOrder = e => {
        ...
        console.log(newOder)
        fetch('https://damp-mesa-00086.herokuapp.com/order', {
            method: 'POST',
            headers: {
    ...
    useEffect(() => {
        fetch(`https://damp-mesa-00086.herokuapp.com/services/${Id}`)
            .then(res => res.json())
            .then(data => {
                setService(data)
                setLoading(false)
    ...
    return (
        <div>
            <h1 className="pt-5 text-center">Order Purchase</h1>
            <div className="d-md-flex container">
                <div className="card col-md-6 col-12 col-sm-12 text-center py-2 my-5" >
                    <img className="img-fluid purchase-image mb-5" src={service?.img} alt="..."></img>
                    <div className="card-body">
                        <h4 className="card-title">{service?.name}</h4>
                        <h6>Price: $ {service?.price}</h6>
                        <p className="card-text mx-3">{service?.des}</p>
                        <Link to="/home" ><button className="btn-purchase"> Back to Purchase </button></Link>
                    </div>
                </div>
                <div className="col-md-6 col-12 col-sm-12 py-2  m-auto my-5 addService">
                    <form onSubmit={handleOrder}>
                        ...
                        <input type="submit" className="btn-purchase" value="Purchases Confirm" />
                    </form>
